...
const EmployeeAttendance = () => {
  ...
  const loadEmployees = async () => {
    try {
      const response = await fetch('https://apartment-v69r.onrender.com/api/employees');
  ...
  const checkIn = async (employeeId) => {
    try {
      const response = await fetch('https://apartment-v69r.onrender.com/api/checkin', {
        method: 'POST',
  ...
  const checkOut = async (employeeId) => {
    try {
      const response = await fetch('https://apartment-v69r.onrender.com/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ EmployeeID: employeeId })
      });
