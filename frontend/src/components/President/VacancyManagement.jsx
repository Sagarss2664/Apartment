...
const VacancyManagement = () => {
  ...
  const fetchVacancies = async () => {
    try {
      const response = await fetch('https://apartment-v69r.onrender.com/api/vacancies');
      if (!response.ok) {
  ...
  const deleteVacancy = async (id) => {
    try {
      const response = await fetch(`https://apartment-v69r.onrender.com/api/vacancies/${id}`, { method: 'DELETE' });
      if (response.ok) {
